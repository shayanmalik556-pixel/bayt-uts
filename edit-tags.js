(function () {
  ...
  function replaceTagInJobTags(oldLabel, newLabel, categoryId) {
    var data = loadJobTags();
    Object.keys(data).forEach(function (jobId) {
      data[jobId] = (data[jobId] || []).map(function (t) {
  ...
  function moveJobTagsToCategory(sourceCatId, targetCatId) {
    var data = loadJobTags();
    Object.keys(data).forEach(function (jobId) {
      data[jobId] = (data[jobId] || []).map(function (t) {
  ...
  function removeTagFromAllJobs(categoryId, tagLabel) {
    var data = loadJobTags();
    Object.keys(data).forEach(function (jobId) {
      data[jobId] = (data[jobId] || []).filter(function (t) {
  ...
  function removeCategoryFromAllJobs(catId) {
    var data = loadJobTags();
    Object.keys(data).forEach(function (jobId) {
      data[jobId] = (data[jobId] || []).filter(function (t) { return t.categoryId !== catId; });
    });
    saveJobTags(data);
  }
  ...
  function showConfirm(message, onConfirm, onCancel) {
    var overlay = document.getElementById('edit-confirm-overlay');
    var msgEl = document.getElementById('edit-confirm-message');
    var okBtn = document.getElementById('edit-confirm-ok');
    var cancelBtn = document.getElementById('edit-confirm-cancel');
  ...
  function showAlert(message) {
    var overlay = document.getElementById('edit-alert-overlay');
    var msgEl = document.getElementById('edit-alert-message');
    var okBtn = document.getElementById('edit-alert-ok');
    if (!overlay || !msgEl) return;
    msgEl.textContent = message;
    function close() {
      overlay.setAttribute('hidden', '');
  ...
  function showRenameModal(labelText, currentValue, onSave, onCancel) {
    var overlay = document.getElementById('edit-rename-overlay');
    var labelEl = document.getElementById('edit-rename-label');
    var inputEl = document.getElementById('edit-rename-input');
    var saveBtn = document.getElementById('edit-rename-save');
    var cancelBtn = document.getElementById('edit-rename-cancel');
  ...
  function showMergeModal(title, options, onConfirm) {
    var overlay = document.getElementById('edit-merge-overlay');
    var titleEl = document.getElementById('edit-merge-title');
    var selectEl = document.getElementById('edit-merge-select');
    var confirmBtn = document.getElementById('edit-merge-confirm');
    var cancelBtn = document.getElementById('edit-merge-cancel');
  ...
  function deleteCategory(catId) {
    ...
    removeCategoryFromAllJobs(catId);
    saveCustom();
  }
  ...
  function mergeCategoryInto(sourceCatId, targetCatId) {
    ...
    moveJobTagsToCategory(sourceCatId, targetCatId);
  ...
  function deleteTag(catId, tagLabel) {
    ...
    removeTagFromAllJobs(catId, tagLabel);
    saveCustom();
  }

  function mergeTagInto(catId, sourceLabel, targetLabel) {
    if (sourceLabel === targetLabel) return;
    replaceTagInJobTags(sourceLabel, targetLabel, catId);
    var list = customData.tagsByCategory[catId] || [];
  ...
  function closeActionsDropdown() {
    var dd = document.getElementById('edit-actions-dropdown');
    if (dd) {
  ...
  function runAction(action) {
    ...
    if (contextType === 'category') {
      if (action === 'rename') {
        ...
        showConfirm('Delete category "' + getCategoryLabel(catId) + '"? All tags in this category will be removed from every job. Continue?', function () {
          deleteCategory(catId);
          render();
        });
      } else if (action === 'merge') {
        var sourceLabel = getCategoryLabel(catId);
        showConfirm('All jobs tagged with tags from "' + sourceLabel + '" will be moved to the category you select. Continue?', function () {
      ...
      if (action === 'rename') {
        ...
        if (i >= 0) {
          showRenameModal('New tag name', tagLabel, function (name) {
            if (name) {
              list[i] = name;
              customData.tagsByCategory[catId] = list;
              replaceTagInJobTags(tagLabel, name, catId);
              saveCustom();
              render();
            }
          });
        ...
        showConfirm('Delete tag "' + tagLabel + '"? This tag will be removed from all jobs. Continue?', function () {
          deleteTag(catId, tagLabel);
          render();
        });
      } else if (action === 'merge') {
        showConfirm('All jobs with tag "' + tagLabel + '" will be moved to the new tag you select. Continue?', function () {
  ...
  function setupCloseDropdownOnOutsideClick() {
    document.addEventListener('click', function (e) {
      if (e.target.closest('#edit-actions-dropdown') || e.target.closest('.edit-btn-menu')) return;
      closeActionsDropdown();
    });
  }

  function initAddCategory() {
    var form = document.getElementById('edit-add-category-form');
    var input = document.getElementById('edit-add-category-input');
    var submit = document.getElementById('edit-add-category-submit');
    var cancel = document.getElementById('edit-add-category-cancel');
    if (!form) return;
